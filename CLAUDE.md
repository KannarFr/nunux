...
| `bin/*` | helper scripts (`osd`, `battery-watch`, `powermenu`, `restic-backup`, `migrate-home`, `pkglist-refresh`, `tcc-profile`, `charge-profile`, `tcc-snapshot`, `yubikey-glow`, `gpg-sign-glow`, `make-swapfile`) referenced by absolute path (sway's `set $bin`, systemd units, pacman hooks, waybar `on-click`, gitconfig's `gpg.program`) — not symlinked, not on `$PATH` |
